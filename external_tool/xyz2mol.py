...
def main(filename, charged_fragments, quick, embed_chiral, use_huckel, output_format, charge) :
    # read atoms and coordinates. Try to find the charge

    atoms, charges, xyz_coordinates = read_xyz_file(filename)
    ...
    mol = xyz2mol(atoms, xyz_coordinates,
                  charge=charge,
                  use_graph=not quick,
                  allow_charged_fragments=not charged_fragments,
                  embed_chiral=not embed_chiral,
    ...
    if output_format == "sdf" :
        ...
        isomeric_smiles = not embed_chiral
